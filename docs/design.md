...
*阻止者在网络中大量部署监控节点，如果发现有对该数据的询问，即“立刻”响应并回复*。从而占据响应**原路局返回&收敛**的头部位置，干扰甚至阻止有效回复抵达询问者。
...
这样，对于快速返回虚假回复的攻击者来说，它们的回复会被上级节点延时回传，从而失去时间优势。

当然所有的末端都会延时，但攻击者的*快速反应*就失去了价值。

这不会导致每一级回传的延时，因为对于先期转播询问的节点，时间早已标记。只有临近数据源时，延时才会产生效果。

> **注：**
> 延时主要用于弥补真实数据源查询自身时花费的时间。
