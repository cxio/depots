...
> **提示：**
> 驿站提供的数据服务也可以不是数据，而就是服务（比如聊天）。在数据网络中请求的数据ID此时只是一个入口标识。
...
在回复包的逆向回传路径上，每一层节点都会收到多个回复，但它们向上级续传时，只会发送一个回复包。

出于安全性考虑，节点不应当只是简单返回最先收到的回复，然后丢弃剩下的回复，而是应当有一个策略：
...
一个大尺寸数据通常采用分片传输，多个分片哈希的汇总哈希称为数据ID。这与从文件内容本身计算的哈希摘要（文档ID）不同。

询问请求里的数据索引通常是指数据ID，即便内部包含结构。
