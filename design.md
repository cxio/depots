...
数据网络存储的是所有愿意将其上传至数据网络的任意数据，它是开放性的。逻辑上，这样的数据规模是无限的，因此每一个驿站肯定会有自己的选择策略。
...
用一个函数调用来反应，类似于白名单的角色：函数返回真，则表示需要存储，否则忽略。

每个类别都有一个策略函数。函数接受目标数据的基本信息：索引和大小。通常你只需要定义数据ID的匹配式即可。但有时你需要有特别的选择。策略函数支持两种语言编写：Go原生语言、Lua脚本。

每一个数据类别的策略定制都存放在以其值为目录名的目录内，比如 0、1（它们都是目录名）。在其中有黑白名单的 `.json` 定义文件，以及所支持的两种语言的代码文件 `ploy.go` 和 `ploy.lua`。其中 `.go` 优先于 `.lua` 处理。
