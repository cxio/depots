...
在逻辑上，数据网络存储的是所有愿意将其上传至数据网络的任意数据，它是开放性的。

这样的数据规模是无限的（至少逻辑上如此），因此每一个驿站肯定会有自己的选择性存储。目前，这由三个项措施来达成：**策略函数**、**白名单**、**黑名单**。


### 策略函数集

可以用一个或多个函数来反应驿站的存储策略。通常，函数接受目标数据的基本信息为实参，包括：类别、索引和大小。

不同策略函数之间是“or”的关系，即任何一项满足即可。策略函数应当是可配置的。


### 白名单与黑名单

有一些数据可能是节点想无条件保留的，不论策略函数是否返回真。虽然数据网络对其中流动数据的分辨被设计得很模糊（隐私性考虑），但白名单依然可行。白名单通常是具体的数据ID，也可支持通配符匹配。

数据网络鼓励无差别随机存储，这样对数据在全网的均匀分布有利，但肯定会有用户不愿意存储某些东西，这就是黑名单的需求。

在技术上，黑名单与白名单类似，依然只能相对于数据ID，暂不支持函数判断的能力。

在优先级上，白名单优于黑名单，如果同一份数据ID同时存在于黑名单和白名单中，则白名单有效（黑名单被忽略）。其次，黑名单优于策略函数，如果某一数据已经存在于黑名单中，则不会再执行策略函数判断。
