...
借鉴于Tor链路的中转思想，我们可以设计一个第三方应用：*数据中转服务网络*。这是一个通用的子网，客户端相互连接，提供数据中转服务。因为服务直接面对用户，所以它可以向用户提供自己的代币收益地址，就像普通的数据节点一样。
...
一个大尺寸的数据可能采用分片存储，多个分片哈希的汇总哈希称为数据ID。这与从文件内容本身计算的哈希摘要（文档ID）不同。

询问请求里的数据索引通常是指数据ID，即便内部包含结构。如果文档未分片，其数据ID和文档ID是同一个。
